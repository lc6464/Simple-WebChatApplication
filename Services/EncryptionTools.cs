...
public class EncryptionTools : IEncryptionTools {
	...
	public bool TryDecryptUserData(string[] input, out RegisterUserDataOutput? output) {
		...
		//_logger.LogDebug("sign: {}", Convert.ToBase64String(signature));
		//_logger.LogDebug("ts: {}", BitConverter.ToInt64(timestamp).ToString());
		...
		//_logger.LogDebug("UserData: {}", JsonSerializer.Serialize(userData));
		...
		var signData = sha256.ComputeHash(signRawData); // 计算签名数据
		//_logger.LogDebug("signData: {}", Convert.ToBase64String(signData));
