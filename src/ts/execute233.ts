...
export async function getBingImageUrl() {
    ...
    try {
        const {success, result, message} = await fetchData( 'https://cn.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1&mkt=zh-CN',{method: 'get'});
        if (result.ok) {
...
export async function loadBackgroundImageFromBing() {
    ...
    if (url != null) {
        document.body.style.background = 'url(' + url + ');background-size: 100% ;background-attachment: fixed';
    }
}